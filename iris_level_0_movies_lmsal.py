...
parser.add_argument('--begin','-b',help='Start yyyy/mm/dd/Hhhmm; ingnored if --hours is set.',default='2024/12/06/H0000')
parser.add_argument('--end','-e',help='End yyyy/mm/dd/Hhhmm; ingnored if --hours is set.',default='2024/12/06/H0100')
parser.add_argument('--hours',help='Try to create movies that many hours into the past from now',type=int)
parser.add_argument('--l0path',help='Path to Level 0 data',default='/irisa/data/level0/')
...
for date in dates:
    ...
    for band, fits_files in [
        ...
        if not os.path.exists(fullpath):
            movie_maker = l0mr.FITSMovieMaker(fits_files=fits_files,output_path=fullpath)
            movie_maker.create_movie()
        else: logger.info(fullpath+' exist.')
